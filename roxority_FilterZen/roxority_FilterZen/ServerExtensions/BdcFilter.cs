...
using Microsoft.SharePoint.BusinessData.Infrastructure.Collections;
...
using BdcField = Microsoft.Office.Server.ApplicationRegistry.MetadataModel.Field;
using BdcFilterCollection = Microsoft.Office.Server.ApplicationRegistry.Runtime.FilterCollection;
using BdcLobSysInst = Microsoft.Office.Server.ApplicationRegistry.MetadataModel.LobSystemInstance;
using BdcLobSysInstDict = Microsoft.Office.Server.ApplicationRegistry.MetadataModel.NamedLobSystemInstanceDictionary;
...
using BdcField = Microsoft.BusinessData.MetadataModel.IField;
using BdcFilterCollection = roxority_FilterZen.ServerExtensions.BdcFilter.FilterCollection;
using BdcLobSysInst = Microsoft.BusinessData.MetadataModel.ILobSystemInstance;
using BdcLobSysInstDict = Microsoft.BusinessData.MetadataModel.Collections.INamedLobSystemInstanceDictionary;
...
namespace roxority_FilterZen.ServerExtensions {
	...
	public class BdcFilter : FilterBase.Interactive, IDisposable {
...
#if !SP12
		internal class FilterCollection : ReadOnlyCollection<IFilter>, IFilterCollection {
		}
#endif
...
#if !SP12
		private static BdcService bdcService = null;
#endif
...
#if !SP12
		private DatabaseBackedMetadataCatalog bdcCat = null;
#endif
		private BdcLobSysInst lobInstance = null;
		private BdcEntity entity = null;
		private BdcView view = null;
		private BdcField valueField = null, dispField = null;
		...
		internal Dictionary<string, BdcEntity> GetEntities (BdcLobSysInst inst) {
#if SP12
			return inst.GetEntities ();
#else
			Dictionary<string, BdcEntity> dict = new Dictionary<string, BdcEntity> ();
			foreach (BdcEntity ent in inst.GetEntities ())
				dict [ent.Name] = ent;
			return dict;
#endif
		}
...
#else
			return inst.GetIdentity ().GetIdentifierValues ();
#endif
		}
		...
		protected internal override IEnumerable<string> AllPickableValues {
			get {
				...
				if ((LobInstance != null) && (Entity != null) && (View != null) && (ValueField != null))
					using (IEntityInstanceEnumerator values = Entity.FindFiltered (new BdcFilterCollection (), LobInstance))
						while (values.MoveNext ())
							if ((values.Current != null) && ((val = values.Current [ValueField.Name]) != null))
				...
				if ((dispField == null) && (View != null))
					try {
						if (string.IsNullOrEmpty (bdcDisplayField))
							...
							foreach (BdcField f in View.Fields)
...
#else
						 foreach(BdcEntity ent in lobInstance.GetEntities())
							 if (ent.Name == bdcEntity.Substring (pos + SEPARATOR.Length)) {
								 entity = ent;
								 break;
							 }
#endif
...
#else
								inst = Entity.FindSpecific (new Identity (idVal), LobInstance);
#endif
							} catch (Exception ex) {
								Report (ex);
							}
		...
		public BdcLobSysInst LobInstance {
			get {
				string bdcEntity = Get<string> ("BdcEntity");
				int pos = ((bdcEntity == null) ? -1 : bdcEntity.IndexOf (SEPARATOR));
				if ((lobInstance == null) && (pos > 0))
					try {
						if ((lobInstance = GetLobSystemInstanceByName (bdcEntity.Substring (0, pos))) == null)
							throw new Exception (this ["BdcFailed", bdcEntity]);
					} catch {
						throw new Exception (this ["BdcFailed", bdcEntity]);
					}
