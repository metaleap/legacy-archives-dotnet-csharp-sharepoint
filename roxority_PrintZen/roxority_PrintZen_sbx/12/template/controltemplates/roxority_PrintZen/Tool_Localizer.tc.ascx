...
<%@ Assembly Name="roxority_PrintZen, Version=1.0.0.0, Culture=neutral, PublicKeyToken=68349fdcd3484f01" %>
...
<script runat="server">
	...
	public string LocKey {
		get {
			return ((SelectedTab == "Studio") ? "roxority_Shared" : ("roxority_PrintZen_" + SelectedTab));
		}
	}
...
<div style="position: fixed; top: 136px; left: 707px; width: 175px; text-align: center; background-color: InfoBackground; color: InfoText; border: 1px solid #909090; padding: 0px 8px 0px 8px; display: <%= (ProdPage.IsAdminSite ? "none" : "block")%>; <%= ProdPage.IsAdminSite ? string.Empty : "font-size: 11px; line-height: 1.3em; padding: 4px"%>" id="roxlocchanges">
	...
	%>
	<a href="<%= ProductPage.MergeUrlPaths (ProdPage.AdminSite.Url, "_layouts/roxority_PrintZen/" + ProdPage.GetLink (null))%>"><b><%= this ["Tool_NoAdminLink"]%></b></a>
	<%
		}
	%>
</div>
...
<table cellpadding="0" cellspacing="0">
	...
	<tr>
		<td colspan="3" class="rox-tabcell">
			<b style="display: block; padding-left: 20px; background: url('/_layouts/roxority_PrintZen/img/flag/<%= SiteLanguage%>.png') no-repeat left center;"><%= (DictComparer.UiResources.Contains (key) ? "[UI] " : string.Empty) + Server.HtmlEncode (key)%>:</b>
			<pre id="pre_<%= key%>" style="padding: 0px 4px; margin-top: 8px; margin-bottom: 8px;">
				<%= Server.HtmlEncode (resources [key])%>
			</pre>
			<b style="display: block; padding-left: 20px; background: url('/_layouts/roxority_PrintZen/img/flag/<%= SelectedLanguage%>.png') no-repeat left center;"><%= SelectedCulture.DisplayName%>: <a id="addlink_<%= key%>" href="#noop" onclick="roxAddLoc('<%= key%>');" style="display: <%= (hasLoc ? "none" : "inline")%>;"><%= this ["Tool_Localizer_Add"]%></a><a id="clearlink_<%= key%>" href="#noop" onclick="roxClearLoc('<%= key%>');" style="display: <%= (hasLoc ? "inline" : "none")%>;"><%= this ["Tool_Localizer_Clear"]%></a>&nbsp;&nbsp;<a id="googlink_<%= key%>" href="#noop" onclick="roxGoogLoc('<%= key%>');" style="font-weight: normal; display: <%= (hasLoc ? "none" : "none")%>;">Google Translate</a></b>
...
<script type="text/javascript" language="JavaScript" src="<%= ProductPage.MergeUrlPaths (SPContext.Current.Web.Url, "_layouts/roxority_PrintZen/mash.tl.aspx?op=gj&v=" + ProductPage.Version)%>" xsrc="<%= Request.IsSecureConnection ? "https" : "http"%>"://www.google.com/jsapi?roxv=<%= ProductPage.Version%>">
</script>
<script type="text/javascript" language="JavaScript">
try {
