...
%>
	...
	function roxRefreshFieldList(ctlID) {
		var val, pn, pos, dsid = ctlID.substr(0, ctlID.indexOf('_')), prefix = roxDataTypePrefixes[roxGetCtlVal(dsid, 't')], $sel = jQuery('select.rox-iteminst-fieldsel-' + dsid), url = '<%= SPContext.Current.Web.Url%>/_layouts/$$$PROJECTNAME$$$/mash.tl.aspx?op=rf&ss=<%= (Schema.Owner.SiteScope ? 1 : 0)%>&dsid=' + dsid, onFinish = function() {
