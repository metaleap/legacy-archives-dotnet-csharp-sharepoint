...
namespace Ionic.Zip
{
    ...
    IDisposable
    {
		public event EventHandler DirectoryError, FileError;
		public event System.ComponentModel.CancelEventHandler InclusionFilter;
	...
	/// </para>
        ...
        private ZipEntry AddOrUpdateDirectoryImpl(string directoryName, string rootDirectoryPathInArchive, AddOrUpdateAction action, int level)
        {
			...
			if (InclusionFilter != null)
				InclusionFilter (directoryName, args);
...
#if SILVERLIGHT
			IEnumerable<string> filenames = System.IO.Directory.EnumerateFiles (directoryName);
#else
            IEnumerable<string> filenames = System.IO.Directory.GetFiles(directoryName);
#endif
            // add the files: 
			if (filenames != null)
				foreach (String filename in filenames)
...
#if SILVERLIGHT
			IEnumerable<string> dirnames = System.IO.Directory.EnumerateDirectories (directoryName);
#else
			IEnumerable<string> dirnames = System.IO.Directory.GetDirectories (directoryName);
#endif
			if (dirnames != null)
				foreach (String dir in dirnames)
					try {
						...
						if (DirectoryError != null)
							DirectoryError (ex, EventArgs.Empty);
						else
							throw;
					}
			//_contentsChanged = true;
        ...
        public void Save()
        {
            try
            {
				...
				foreach (ZipEntry e in _entries) {
					...
					if ((e != null) && (e.OutputUsedZip64 != null) && e.OutputUsedZip64.HasValue)
...
#if SILVERLIGHT
						(zf._provisionalAlternateEncoding.WebName == "IBM437")
#else
						(zf._provisionalAlternateEncoding.CodePage == 437)
#endif
        ...
        public System.Collections.ObjectModel.ReadOnlyCollection<string> EntryFileNames
        {
            get
            {
				Converter<ZipEntry, string> converter = delegate (ZipEntry e) {
					return e.FileName;
				};
#if SILVERLIGHT
				System.Collections.Generic.List<string> foo = new System.Collections.Generic.List<string> (_entries.Count);
				foreach (ZipEntry e in _entries)
					foo.Add (converter (e));
#else
				var foo = _entries.ConvertAll (converter);
#endif
