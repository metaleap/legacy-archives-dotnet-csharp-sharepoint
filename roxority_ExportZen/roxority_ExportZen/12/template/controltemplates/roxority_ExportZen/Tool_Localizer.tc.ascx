...
<table cellpadding="0" cellspacing="0">
	...
	<tr>
		<td colspan="3" class="rox-tabcell">
			...
			<b style="display: block; padding-left: 20px; background: url('/_layouts/roxority_ExportZen/img/flag/<%= SelectedLanguage%>.png') no-repeat left center;"><%= SelectedCulture.DisplayName%>: <a id="addlink_<%= key%>" href="#noop" onclick="roxAddLoc('<%= key%>');" style="display: <%= (hasLoc ? "none" : "inline")%>;"><%= this ["Tool_Localizer_Add"]%></a><a id="clearlink_<%= key%>" href="#noop" onclick="roxClearLoc('<%= key%>');" style="display: <%= (hasLoc ? "inline" : "none")%>;"><%= this ["Tool_Localizer_Clear"]%></a>&nbsp;&nbsp;<a id="googlink_<%= key%>" href="#noop" onclick="roxGoogLoc('<%= key%>');" style="font-weight: normal; display: <%= (hasLoc ? "none" : "none")%>;">Google Translate</a></b>
