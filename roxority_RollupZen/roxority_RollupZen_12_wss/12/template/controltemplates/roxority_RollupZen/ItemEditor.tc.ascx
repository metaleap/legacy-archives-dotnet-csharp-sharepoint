...
<%
		foreach (int lcid in ProductPage.WssInstalledCultures)
			if (!lcidsDone.Contains (lcid)) {
				lcidsDone.Add (lcid);
%>
<link rel="stylesheet" type="text/css" href="/_layouts/<%= lcid%>/styles/Themable/layouts.css"/>
<%
			}
		try {
...
%>
<link rel="stylesheet" type="text/css" href="/_layouts/<%= ctx.Web.RegionalSettings.LocaleId%>/styles/Themable/layouts.css"/>
<%
			} else if ((ctx.Web.Locale != null) && !lcidsDone.Contains (ctx.Web.Locale.LCID)) {
				lcidsDone.Add (ctx.Web.Locale.LCID);
%>
<link rel="stylesheet" type="text/css" href="/_layouts/<%= ctx.Web.Locale.LCID%>/styles/Themable/layouts.css"/>
<%
			}
		} catch {
		}
...
<script language="JavaScript" type="text/javascript">
	var roxDefTab = '', roxAllTabs = [], roxSelTabs = {}, roxHasChanges = false, roxShowIfs = {}, roxDataTypePrefixes = {}, roxItemIDs = [];

<%
	foreach (Type t in DataSource.KnownProviderTypes) {
%>
	roxDataTypePrefixes['<%= t.Name%>'] = '<%= DataSource.GetStatic (t).SchemaPropNamePrefix%>';
<%
	}
%>

	function roxAutoHideProps(itemID) {
		var allTrue, vals, curVal, visCount;
		for (var p in roxShowIfs) {
			allTrue = true;
			for (var sp in roxShowIfs[p]) {
				vals = jQuery.isArray(roxShowIfs[p][sp]) ? roxShowIfs[p][sp] : [roxShowIfs[p][sp]];
				curVal = jQuery('#' + itemID + '_' + sp).val();
				if (jQuery.inArray(curVal, vals) < 0) {
					allTrue = false;
					break;
				}
			}
		...
		for (var i = 0; i < roxAllTabs.length; i++)
			if ((pos = roxAllTabs[i].indexOf('__')) > 0) {
				...
				for (var j = 0; j < roxItemIDs.length; j++)
					jQuery('a#roxiteminsttab_' + roxItemIDs[j] + '_' + roxAllTabs[i]).css({ "display": ((tt == jQuery('#' + roxItemIDs[j] + '_' + tp).val()) ? "inline-block" : "none") });
			}
	}

	function roxGetDynInst(dsid, noEnc) {
		var pn, pos, prefix = roxDataTypePrefixes[jQuery('#' + dsid + '_t').val()], dyn = {};
		<%
			foreach (JsonSchemaManager.Property prop in Schema.Properties)
		...
		%>
		if ((((pos = (pn = '<%= prop.Name%>').indexOf('_')) <= 0) || (pn.substr(0, pos) == prefix)) && ((val = jQuery('#' + dsid + '_<%= prop.Name%>').val()) !== undefined))
			dyn['<%= prop.Name%>'] = val;
		<%
				}
		%>
	...
	function roxRefreshFieldList(ctlID) {
		var val, pn, pos, dsid = ctlID.substr(0, ctlID.indexOf('_')), prefix = roxDataTypePrefixes[jQuery('#' + dsid + '_t').val()], $sel = jQuery('select.rox-iteminst-fieldsel-' + dsid), url = '<%= SPContext.Current.Web.Url%>/_layouts/roxority_RollupZen/mash.tl.aspx?op=rf&ss=<%= (Schema.Owner.SiteScope ? 1 : 0)%>&dsid=' + dsid, onFinish = function() {
...
<div class="rox-iteminst" id="roxiteminst_<%= inst ["id"]%>" style="display: <%= ("new".Equals (inst ["id"]) ? "none" : "block")%>;">
	...
	<div class="rox-iteminst-edit-box" style="display: none;" id="roxiteminsteditbox_<%= inst ["id"]%>">
		...
		<div class="rox-iteminst-edit-prop rox-iteminst-edit-<%= prop.Tab%>" id="roxinstprop_<%= inst["id"]%>_<%= prop.Name%>" style="display: none; <%= (lastTab != prop.Tab) ? "padding-top: 12px;" : string.Empty%>" onmouseover="jQuery('#roxiteminsthelp_<%= inst ["id"]%>_<%= prop.Name%>').css({ 'visibility': 'visible' });" onmouseout="jQuery('#roxiteminsthelp_<%= inst ["id"]%>_<%= prop.Name%>').css({ 'visibility': 'hidden' });">
			...
			<%
				if (!string.IsNullOrEmpty (tmp = prop.PropertyType.RenderValueForEdit (prop, inst, !prop.Editable))) {
			%>
